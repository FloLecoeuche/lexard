...
7. If any fail:
   ...
   - Next action: "Fix the failing criteria, then run `/flo_continue --complete` again"
...
5. Create feature branch `feature/us-$ARGUMENTS-<short-name>`
...
9. Next action: "Continue implementing tasks. Run `/flo_continue` to check progress."
...
5. **If ALL tasks appear complete:**
   ...
   - Next action: "Run `/flo_continue --complete` to verify and finalize"
6. **If tasks remain:**
   - Show: "Tasks: X/Y complete"
   - List remaining tasks with checkboxes
   - Continue implementing next incomplete task
   - Next action: "Continue with remaining tasks, or `/flo_status` to check progress"
...
3. Next action: "Run `/flo_continue X.X` to start this US"
